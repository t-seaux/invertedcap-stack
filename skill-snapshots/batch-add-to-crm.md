...
5. Restates Tom's standing rules that don't transit to spawned sub-agents automatically:
   ...
   - **Founder-background vs pitched-company conflation** — when the per-company blurb describes a founder's background ("ex-CTO of Stripe, now building Acme"), the Opportunity is the company being pitched NOW (Acme). Prior employers / "ex-X" credentials go in the Description and page-body Team background, never as the Opp title or identity.

...
2. **Create the Opportunity — fast path, from the store row (do NOT run `add-to-crm`'s enrichment inline).** The store row is ALREADY fully enriched by `neg1-enricher` (arc, email, eval, employers) — re-running `add-to-crm`'s ContactOut re-enrichment + synchronous per-employer Companies-DB treatment is redundant work that blows this job's 600s timeout (the 2026-08-03 Charlie Schwartz job was killed at 605s mid-run, which is what spawned the duplicate drafts + duplicate opp). Instead create the opp DIRECTLY via `notion-create-pages` using the canonical -1 pre-company field table (pipeline-agent Task 7 step 3b): Name `-1 ([{Name}]({LI URL}))` — **parens around the LINKED name** (Tom, 2026-07-27; validate with `validate_eval_note.py --opp-title "<Name md>"`), Stage `Pre-Seed 💡`, **Source(s) = PROVENANCE-AWARE — read the store row's `source` field first** (Tom, 2026-08-29, Charles Wong: "Source should actually be primary and not Claude here"). TWO shapes, no registry (Tom, same day: "primary and Fika have their own email lookup in people db so why do we need registered slug"): (1) `source` is an **email address** → the sender was an external referrer, human OR machine — resolve against the **People DB by Email** (same lookup add-to-crm uses for referrers; Primary Venture Partners = `deal-agent@primary-os.com` → `3c200bef-f4aa-8007-9fb5-d6680db3702c`, Fika Ventures = `investments@fika.vc` → `3c200bef-f4aa-8134-8b3b-d5ca8e46bc0c`) → Source(s) = that row. **Named engine-operator alias (Tom, 2026-09-24):** `liam@levelvc.com` → **Level Ventures** (`3e500bef-f4aa-81a6-9d79-f076a83c1174`), NOT Liam's own row. That row deliberately has no email, so apply this override BEFORE the People-DB email lookup. Liam counts as the Source only if he explicitly introduced Tom to the founder. The engine only *qualified* the candidate, it didn't *find* them — provenance survives qualification. Email not found → fall back to Claude AND flag the gap in the close-loop reply; NEVER auto-create the person. (2) anything else = engine-internal (weekly sweep, recipes A–F, wildcard, structured, network deep sweep, manual "-1 scan") → **the `Claude` source page** (relation to `07500bef-f4aa-8213-a8f8-01cfa3cb9a12` — Tom, 2026-07-27: "any candidate sourced through claude... set source as claude"; never `Direct` for engine-sourced candidates; `0fb9a640-34fd-46f9-9347-68d590e69dc9` is `Direct`'s page ID, not `Claude`'s — a 2026-08-03 copy-paste bug wrongly used it here, mislinking every -1 Opp created via this path until fixed), Fund `Inverted 1️⃣`, **Status `Qualified` — NOT `Outreach`** (Tom, 2026-07-27, Nina Carriero: "it should be added to qualified; only if i actually reach out to her should you add to outreach" — a draft is not outreach; pipeline-agent Task 7 flips Qualified → Outreach at SEND time when it detects the email went out), Description describes the **newco/company**, not the person (founder bio lives in People-DB, not here) — `TBD` (bare) unless a text/email surfaced an actual signal on what they're building next (e.g. `"Exploring something in construction."`) (Tom, 2026-08-12), Contact = email, Website N/A, 🏁 Founder(s) = resolve/create the founder People DB entry inline (needed for the relation; this is fast), **Icon = 🌱** (pre-company convention — the founder has no company/logo yet; add-to-crm's normal opps get a thematic company emoji, but this fast path skipped icons entirely until 2026-08-12, when a Notion gallery card shipped with a blank icon).
   ...
   **Post-create field verification (2026-08-12) — `notion-fetch` the new page and diff EVERY field-table value against what actually landed**, not just spot-check the relations/status. The Yu Zhou Lee Opp (2026-08-11) shipped with `Description` blank despite the table saying `TBD` and the run's own prose confirming it — a plain literal placeholder value silently didn't make it into the `notion-create-pages` call. Placeholder/boilerplate values (`Description: TBD`, `Website: N/A`, `Icon: 🌱`) are the easiest to drop because nothing downstream errors on them being blank — verify them explicitly, don't assume "the table said so" means it happened.

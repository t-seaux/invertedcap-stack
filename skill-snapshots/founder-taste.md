...
name: founder-taste
description: |-
  Tom's founder-taste corpus + the synthesis entry point for questions about his own investing style. TRIGGER on "what's my investing style", "what's my founder taste", "why do I pass", "why do I invest", "what do I actually look for", "how has my taste changed", "what are my blind spots" — Query Mode. Otherwise this is a corpus, not a workflow: never trigger it for drafting or pipeline tasks. Single home for the doctrine and evidence of how Inverted Capital evaluates founders: RUBRIC.md (corpus index — thesis + 6-signal rubric + 4 archetypes + scoring), SYSTEM.md (execution system + data model + feedback engines), CASEBOOK.md (worked examples), DECISION_RETROS.md (retro nugget log), PILLARS.md (Argument Pillars), ONLINE_SOURCES.md (research taxonomy); the decision ledger DB is the structured spine. Consumers: neg1-enricher, founder-outreach, decision-retro(+listener), retro-weekly-summary, network-scan, draft-feedback, first-pass-diligence, pipeline-agent, pass-note-drafter. RUBRIC.md edits are human-gated — Tom's explicit call only.

...
- **Title span**: `.sm-retrieval-title` — `font-size: 11px; font-weight: 700; color: #fffffb; letter-spacing: 0.08em; text-transform: uppercase; line-height: 1.2;`. Text: `Retrieval Layer` (no trailing colon — sub follows in parentheses).
- **Sub span**: `.sm-retrieval-sub` — `font-size: 11px; font-weight: 700; color: #fffffb; letter-spacing: 0.06em; text-transform: uppercase; line-height: 1.2;` (no `margin-left` — flex `gap` handles spacing). Text: `(Cache, Enrichment & Embeddings)` (parenthetical, comma + ampersand serial — no Oxford comma since `&` is used). Both spans are the same `#fffffb` white — the entire line matches the title color of the Agentic Workflows pill, NOT the gray-sub treatment used there. (User explicitly requested unified white in 2026-05-07 — do not split colors.)

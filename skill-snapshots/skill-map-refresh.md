...
- **Title span**: `.sm-retrieval-title` — `font-size: 11px; font-weight: 700; color: #fffffb; letter-spacing: 0.08em; text-transform: uppercase;`. Text: `Retrieval Layer` (no trailing colon — sub follows in parentheses).
- **Sub span**: `.sm-retrieval-sub` — `font-size: 11px; font-weight: 700; color: #b1bac4; letter-spacing: 0.06em; text-transform: uppercase; margin-left: 8px;`. Text: `(Cache, Enrichment & Embeddings)` (parenthetical, comma + ampersand serial — no Oxford comma since `&` is used).

...
- **Container**: `.sm-retrieval-strip` — full-width flex row, `display: flex; align-items: baseline; justify-content: center; gap: 8px;`. `margin: 8px 0 14px`, `padding: 5px 10px 4px`, `line-height: 1.2`, `background-color: transparent`, `border: 0.5px solid #30363d`, `border-radius: 8px`. **Fill is a stippled halftone-style dot pattern, not a solid color** — `background-image` layers three patterns: (1) the standard shine `linear-gradient`, (2) a small-dot `radial-gradient(circle, rgba(255,255,255,0.30) 0.3px, transparent 0.7px)` at `background-size: 4px 4px`, position `0 0`, (3) the same dot gradient offset by half a tile (position `2px 2px`) to create a diamond/halftone-staggered grid. `background-repeat: no-repeat, repeat, repeat`. **`background-clip: padding-box;`** — single-value applies to all layers; dots fill the entire interior of the border (no content-box inset) so the bar reads as a fully-filled stippled surface. Hover rule: `background-position: 0% 0, 0 0, 2px 2px` so the shine animates while both dot grids stay pinned. **Padding lineage**: 8/24/7 (24px horiz) → too inset, felt like a center band; 5/10/4 → final padding for text breathing room. **Tile + radius lineage**: 6px/0.8px → clunky; 8px/0.4px → sparse grid-y; 4px/0.5px → still too clunky on retina; 4px/0.3px (alpha bumped to 0.30 to keep visible) → final, fine-grain dense halftone. **The flex baseline alignment is required** — earlier inline-span layout produced subtle baseline misalignment between the white title and the gray sub on monospace fonts.

...
- **Container**: `.sm-retrieval-strip` — full-width flex row, `display: flex; align-items: baseline; justify-content: center; gap: 8px;`. `margin: 8px 0 14px`, `padding: 5px 10px 4px`, `line-height: 1.2`, `background-color: transparent`, `border: 0.5px solid #30363d`, `border-radius: 8px`. **Fill is a stippled halftone-style dot pattern, not a solid color** — `background-image` layers three patterns: (1) the standard shine `linear-gradient`, (2) a small-dot `radial-gradient(circle, rgba(255,255,255,0.38) 0.2px, transparent 0.5px)` at `background-size: 7px 7px`, position `0 0`, (3) the same dot gradient offset by half a tile (position `3.5px 3.5px`) to create a diamond/halftone-staggered grid. `background-repeat: no-repeat, repeat, repeat`. **`background-clip: padding-box;`** — single-value applies to all layers; dots fill the entire interior of the border (no content-box inset) so the bar reads as a fully-filled stippled surface. Hover rule: `background-position: 0% 0, 0 0, 3.5px 3.5px` so the shine animates while both dot grids stay pinned. **Padding**: `5px 10px 4px` for text breathing room. **Tile + radius lineage**: 6px/0.8px → clunky; 8px/0.4px → sparse grid-y; 4px/0.5px → too clunky on retina; 4px/0.3px (α 0.30) → still too dense; **7px/0.2px** (α 0.38) offset-diamond → final, sparse fine-grain stipple. Alpha climbs as radius shrinks to keep tiny dots legible. **The flex baseline alignment is required** — earlier inline-span layout produced subtle baseline misalignment between the white title and the gray sub on monospace fonts.

...
- **Container**: `.sm-retrieval-strip` — full-width flex row, `display: flex; align-items: baseline; justify-content: center; gap: 8px;`. `margin: 8px 0 14px`, `padding: 6px 22px 5px` (tight vertically), `line-height: 1.2`, `background-color: transparent`, `border: 0.5px solid #30363d`, `border-radius: 8px`. **Fill is a dotted texture, not a solid color** — `background-image` layers two patterns: (1) the standard shine `linear-gradient`, (2) a `radial-gradient(circle, rgba(255,255,255,0.11) 0.8px, transparent 1.2px)` with `background-size: 6px 6px` for a subtle stippled look. `background-repeat: no-repeat, repeat` so the shine doesn't tile. Has a SEPARATE hover rule (not the shared one) — sets `background-position: 0% 0, 0 0` on hover so the shine animates while the dots stay pinned. **The flex baseline alignment is required** — earlier inline-span layout produced subtle baseline misalignment between the white title and the gray sub on monospace fonts.

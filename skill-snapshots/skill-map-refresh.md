...
- **Sub span**: `.sm-retrieval-sub` — `font-size: 11px; font-weight: 700; color: #b1bac4; letter-spacing: 0.06em; text-transform: uppercase; line-height: 1.2;` (no `margin-left` — flex `gap` handles spacing). Text: `(Cache, Enrichment & Embeddings)` (parenthetical, comma + ampersand serial — no Oxford comma since `&` is used).

...
- **Container**: `.sm-retrieval-strip` — full-width flex row, `display: flex; align-items: baseline; justify-content: center; gap: 8px;`. `margin: 8px 0 14px`, `padding: 6px 22px 5px` (tight vertically), `line-height: 1.2`, `background-color: transparent`, `border: 0.5px solid #30363d`, `border-radius: 8px`. **Fill is a stippled halftone-style dot pattern, not a solid color** — `background-image` layers three patterns: (1) the standard shine `linear-gradient`, (2) a small-dot `radial-gradient(circle, rgba(255,255,255,0.16) 0.4px, transparent 0.8px)` at `background-size: 8px 8px`, position `0 0`, (3) the same dot gradient offset by half a tile (position `4px 4px`) to create a diamond/halftone-staggered grid. `background-repeat: no-repeat, repeat, repeat`. Earlier 6px tile with larger 0.8px dots read as too clunky; smaller dots in offset-diamond grid feel like fine texture rather than visible markers. Has a SEPARATE hover rule (not the shared one) — sets `background-position: 0% 0, 0 0, 4px 4px` on hover so the shine animates while both dot grids stay pinned. **The flex baseline alignment is required** — earlier inline-span layout produced subtle baseline misalignment between the white title and the gray sub on monospace fonts.

...
- **Container**: `.sm-retrieval-strip` — full-width flex row, `display: flex; align-items: baseline; justify-content: center; gap: 8px;`. `margin: 8px 0 14px`, `padding: 5px 10px 4px`, `line-height: 1.2`, `background-color: transparent`, `border: 0.5px solid #30363d`, `border-radius: 8px`. **Fill is a stippled halftone-style dot pattern, not a solid color** — `background-image` layers three patterns: (1) the standard shine `linear-gradient`, (2) a small-dot `radial-gradient(circle, rgba(255,255,255,0.22) 0.5px, transparent 1px)` at `background-size: 4px 4px`, position `0 0`, (3) the same dot gradient offset by half a tile (position `2px 2px`) to create a diamond/halftone-staggered grid. `background-repeat: no-repeat, repeat, repeat`. **`background-clip: padding-box, content-box, content-box;`** — the dots clip to the content box so they don't run into the border (the shine still extends to the padding edge). Hover rule: `background-position: 0% 0, 0 0, 2px 2px` so the shine animates while both dot grids stay pinned. **Padding lineage**: 8px/24px/7px (24px horiz) → too inset, dots felt confined to a center band. 5px/10px/4px (10px horiz) → final, dots fill nearly the whole bar with a subtle gap from the border. **Tile + radius lineage**: 6px / 0.8px radius → too clunky; 8px / 0.4px radius → sparse grid-y; 4px / 0.3px radius → too small to render at mobile scale; 4px / 0.5px radius offset-diamond → final, dense fine texture that survives mobile scale-down. **The flex baseline alignment is required** — earlier inline-span layout produced subtle baseline misalignment between the white title and the gray sub on monospace fonts.

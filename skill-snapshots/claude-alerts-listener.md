...
0. **🗑️ Archive the deal email** — `args.text` is `🗑️` (the Worker maps a `:wastebasket:` reaction in `#claude-alerts` to it) or Tom's reply is 🗑️ / "archive" / "trash", AND the parent is an add-to-crm deal alert (🆕 / 📌 / 🛡️ carrying a Notion Opp link). Inverted twin of the Dash 🆕-card 🗑️ (sms-listener §4, Tom 2026-09-24). Resolve the Opp from the alert's Notion link → its `Source Thread ID`, then `python3 ~/.claude/scripts/gmail_archive.py <threadId>` (marks every message READ, then archives — drops INBOX; never trash/delete; `already-archived` = success). Do NOT touch the Opp. Close-loop thread reply `🗑️ Archived: <subject>`; `ok:false` → `⚠ Couldn't Archive: <error>`; no `Source Thread ID` on the Opp → `⚠ Nothing to Archive: no Gmail thread on this Opp`. Any other parent → treat 🗑️ as unrecognised feedback and ask.
...
5. **SOI mark confirm** — parent header starts with `📈` AND contains `Mark:` (priced-round mark), or starts with `💸 Exit Distribution:` (valuation-judgment mark; writes `fund_inputs.json` via the engine). Procedure in `references/special-branches.md#special-branch-soi-mark-confirm` — read it now.

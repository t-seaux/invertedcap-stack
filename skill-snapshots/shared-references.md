...
- `product-build-teardown-framework.md` — Canonical six-section framework for analyzing a company's product build: Product Anatomy, Delivery Mechanism, Build Cost & Time to v1, Path to Production-Grade, Moat Read, Failure Modes. Owns structure, depth requirements, table formats, citation discipline, failure mode taxonomy, and the public-surface / engineering-signal / peer-surface crawl methodology. Read by `first-pass-diligence` §2 Product, the standalone `product-build-teardown` skill, and `update-diligence-priors` Product refreshes — single source of truth, no drift across invocation paths.

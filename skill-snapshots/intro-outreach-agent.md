...
Before executing any move in Step 3, every (person, opportunity) candidate must pass all gates below. These apply uniformly across Sub-routine 2A (subject-line scanner), 2B (qualified roster), 2C (forward scanner), and all manual modes.
...
**Gate 4 — Pending Feedback suppression (backchannel ≠ intro outreach).** Fetch the Opp's `📣 Pending Feedback` relation. If the candidate person is currently in `📣 Pending Feedback` for this Opp, **SKIP** — the outbound to them is backchannel diligence outreach (owned by `feedback-outreach-drafter` / `feedback-outreach-scanner`), not an intro attempt. The same person can be a feedback source on one Opp and an intro target on another — this gate is per-(person, opp). Log: `[Person Name] skipped — currently in 📣 Pending Feedback for [Company], outbound treated as backchannel diligence not intro outreach`.
  ...
  **Carve-out — explicit manual promotion.** If Tom's trigger message explicitly says "promote [Name] to intro for [Company]", "move [Name] from feedback to outreach for [Company]", or any equivalent intent to convert a backchannel into a real intro, bypass Gate 4 for that person+opp. In that case, also scrub them from `📣 Pending Feedback` in the same Step 3 atomic write (they leave the feedback lifecycle as they enter the intro lifecycle). This carve-out only applies to manual-mode triggers — sweep and webhook modes never bypass Gate 4.

  Concrete miss: Nick Ling was in Factir's `📣 Pending Feedback` (Tom reached out for his take on the deal as a backchannel); the sweep saw the outbound and wrote him to `☎️ Intros (Outreach)`, misreading backchannel as intro outreach. With Gate 4 in place, the sweep skips and leaves feedback alone.

**Gate 5 — Deal-source skip (the messenger is not the target).** Fetch the Opp's `Source(s)` relation. If the candidate person is in `Source(s)` for this Opp, **SKIP** — Tom's outbound to them is a reply to the person who offered the intro, not outreach to the company itself. This is the deterministic backstop for Gate 1: even if the directionality LLM/heuristic misclassifies an inbound offer as outreach, the Source-relation match forces a skip. Log: `[Person Name] skipped — listed as Source on [Company] (intro offerer, not intro target)`. Concrete miss: Aadik Shekar forwarded Reuben's Agentiq fundraise email offering Tom an intro; Tom's reply to Aadik got written to Agentiq's `☎️ Intros (Outreach)` because the haystack matched "Agentiq". Aadik is in Agentiq's `Source(s)` — Gate 5 catches it.

**Gate 6 — Founder-of-own-Opp skip (a person is not an intro target for their own company).** Reject any candidate who is a founder/operator at the matched Opp's company. Three OR-ed signals constitute a match: (a) the candidate is in the Opp's `🏁 Founder(s)` relation, (b) the candidate's email exactly matches the Opp's `Contact` email graph, (c) the candidate's email domain (non-free-provider) equals the Opp's `Website` domain or any `Contact` domain. If any signal hits, **SKIP** — you don't introduce someone to their own company. Log: `[Person Name] skipped — founder/operator at [Company], not a valid intro target`. Concrete miss: Justin Sherlock (founder of Caspian) got written to Caspian's `☎️ Intros (Outreach)` because Tom emailed him asking for a take on a different Opp (Factir); the matcher hit "Caspian" in the body. Gate 6's domain check (justin@meetcaspian.com matches caspian's Website/Contact domain) catches it.

**Gate 7 — Outreach-context routing (email about a different Opp).** If a candidate clears Gate 6 only because they have no relation to the matched Opp, but the email body more prominently names a *different* Opp from Tom's pipeline (e.g., Tom is pinging a founder-of-Opp-A for diligence feedback on Opp-B), prefer the Opp named in the body as the routing target and treat as feedback outreach (not intro outreach) — add the candidate to `📣 Pending Feedback` on the body's named Opp instead, and skip the intro write on the haystack-matched Opp. Heuristic for "more prominently": the other Opp's name appears in the subject, OR it appears more times in the body than the haystack-matched name. When ambiguous, skip both writes and surface to Needs Review. Concrete miss: Justin (Caspian) was asked about Factir; the matcher hit Caspian via a sentence framing Justin's expertise ("given your duty-drawback work at Caspian"), but Factir was the actual subject. Gate 7 reroutes to Factir feedback rather than logging Caspian intro outreach.

**Gate 8 — Intro-offer intent (peer-ask suppression).** Tom emailing a peer investor / operator for their take on a deal — especially when the recipient was already independently engaged with the company — is NOT intro outreach. The defining shape of intro outreach is that Tom is offering to facilitate a connection. Without an intro-offer signal in Tom's outbound, the message is peer chatter, deal coordination, or a pass-note exchange between two investors who are both independently evaluating. Classify Tom's outbound body as one of: `intro-offer`, `peer-ask`, `other`. Intro-offer signals (any one suffices): explicit phrasing ("happy to intro", "want me to connect you", "should I make the intro", "would love to introduce you to [founder]", "want an intro?", "I can put you in touch", "let me know if you'd like to meet them"), or implicit framing (forwarding a founder's pitch with a clear "let me know if you want to engage" ask). Peer-ask signals: "what do you think", "curious for your take", "have you seen them", "thoughts?", "know anything about", "tracking them?", "any color on", "would love your take". On confident `peer-ask` (≥0.85), **SKIP** the Outreach write entirely. Other verdicts proceed (conservative — better a manual cleanup than a missed legit intro). Mirrors the webhook gate in `~/code/gmail-webhook/Code.js handleIntroOutreach()` via `intro-offer-intent.js`. Concrete miss: Factir 2026-06-03. Tom emailed Steph Khoo (Nyca) about Factir; Steph was already in independent contact with the company and replied with a personal pass note ("definitely too early for us … going on maternity leave"). The webhook wrote her to Outreach; the resolution agent moved her to Declined / NR. Tom never offered an intro. With Gate 8 in place, the peer-ask is detected and no lifecycle entry is created.

**Gate 8b — Talent-forward suppression.** Same classifier, fourth verdict: `talent-forward` = Tom is offering to connect the recipient with a **person looking for a job** (an engineer / operator at a company that is winding down, a candidate between roles) — a hiring or placement favor, not a connection to the Opp's founders. The company named in the body is where the candidate works: it is context for the recipient, not a deal, so its URL still corroborates the Opp match (Tier 1) and the recipient would otherwise land in Outreach for a company Tom never pitched. On confident `talent-forward` (≥0.85), **SKIP** the Outreach write. Concrete miss: Solve Finance 2026-09-22. Tom replied to Bernardo Menezes (who was raising for his own company; Tom declined in the same email) offering Matt Doak — a Solve Finance staff engineer, not the founder (Sean Hundtofte) — as a founding-engineer candidate, linking solve.finance for context. The webhook auto-added Bernardo to Solve's ☎️ Intros (Outreach). Reverted by hand. The gate lives at the shared Opp-match choke point — `findOppByHaystackMatch()` in Code.js returns null on a confident `talent-forward` (regex prefilter → memoized classifier) — so the email is never tagged to that Opp by ANY sent-mail handler (intro-outreach, outreach-decliner, outreach-detector, beneficiary capture). Tom's framing: "the key is NOT to tag this email to the Solve Finance opportunity." Gate 8b inside `handleIntroOutreach()` remains as a second net for talent blurbs the prefilter misses.
...
1. **Fetch the Opportunity page** to get the current state of all five relation fields:
   ...
   - `📣 Pending Feedback` — current JSON array of page URLs (needed for Gate 4 in Step 2.9 and for the manual-promotion carve-out scrub)

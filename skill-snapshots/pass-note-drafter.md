...
- Scan the Opportunity page body for a section labeled **"Pass Note Guidance"** (accept casing/punctuation variants: `pass note guidance`, `Pass-Note Guidance`, `Pass note guidance`, and a trailing parenthetical date such as `Pass Note Guidance (Sept 3, 2026)`). Use `notion-fetch` on the Opp page and walk its block children — do not rely solely on the Description property or a preview snippet, since guidance lives deeper in the body. It normally sits directly beneath the `📚` Company Overview callout, but do not depend on that position — scan the whole body.
...
- **Label format is flexible — match any of these shapes:**
  - A `callout` block (Tom's current default — a `⛔` callout whose first child is the *italic* dated label `Pass Note Guidance (<date>)`, followed by bullets **nested inside the callout**). The guidance content lives as the callout's children, not as sibling blocks after it. `log-pass-note-guidance` writes this shape. **Match on the label text, not its emphasis** — the label is italic here (matching the `📚` Company Overview callout), bolded in the older callout variant, and either is valid. The parenthetical date is a last-updated stamp on the whole section, not a per-bullet timestamp; ignore it for substance.

...
name: neg1-enricher
description: |-
  Enrichment + evaluation primitive for pre-founder (-1) candidates — HEADLESS, upstream of Notion. LinkedIn URL in → enrich (ContactOut + local caches + research) → founder-taste rubric + When gate → verdict written to the CANDIDATE STORE → candidate card posted to #neg1-sourcing for EVERY candidate (Reach Out / Second Look / Pass), one card each, never a consolidated batch. NOTHING lands in Notion — that happens only when Tom replies draft (neg1-sourcing-listener). Job mode (claude-job-queue, args {mode: headless, li_url, source}) is the primary scheduled path; supports --score-only re-scoring. Trigger phrases: "-1 scan [URL]", "scan these profiles", "enrich these LI URLs", "score [name]", "rescore [name]", "run a founder eval on [name/this person]", "founder eval on [X]", or LinkedIn URL(s) with sourcing intent. The -1 Scanner Notion DB is DELETED (2026-07-16) — never query it (archive path in the skill body).
...
**READ `~/.claude/skills/neg1-enricher/EVAL_NOTE_TEMPLATE.md` and render EXACTLY that skeleton** — it supersedes the 2026-07-16/17 "Tier A" spec entirely (Profile section, Signals table, Founder Eval bullets, Other Qualities/Potential Gaps headings, verdict-colored callout, provenance line: ALL RETIRED). The load-bearing shape:
- **Title:** `-1 ([{Name}]({LI URL})): Founder Eval` — parens around the LINKED name; icon `:claude-color:`.
- **Callout** (`gray_bg`, `:claude-color:` icon) = the Slack card 1:1: underlined-bold `Claude Rec.` + verdict WITH emoji, then Presence / Arc / Spike(s) / Gap(s) / Timing bold-led lines. No summary prose, no provenance.
- **`---` dividers** between sections. `#### Eval Summary` (Primary Signal bullet + nested Other Qualities with word ratings + Gaps bullet) → `#### Eval Rationale` (six per-signal bullets, numeric score in each bold lead, `(N/A)` for unobservable — the ONLY place numbers appear) → bold-underlined paragraph `Framework Connections` (Peak + Timing bullets, plain language).
- **HARNESS (never skip):** after creating or editing the note, `notion-fetch` the page and pipe its markdown through `python3 ~/.claude/skills/neg1-enricher/validate_eval_note.py`. Non-zero exit → fix the page in place and re-validate until PASS. A note that fails validation must not be left live.
- Keep the store's `eval_summary`/`eval_rationale` fields for machine use — the note renders the template, not those fields verbatim.
...
- **Step 1.6 (prefilter gate — added 2026-07-20)** — immediately after the ContactOut payload lands, check the full experience array against `~/.claude/skills/founder-taste/PREFILTERS.md`. This is where arc-shape rules become checkable (PF-2 serial short stints, PF-4 mature-enterprise-only arc) and where sourcing-time rules get re-verified against real data (PF-1 investor seat, PF-3 stale-unicorn-only reps). If a HARD rule fires: upsert the store row with `state=passed`, `rec="Pass ❌"`, `working_desc` = one-line kill reason prefixed `[PREFILTERED {rule id}]` — then STOP. No scoring, no card, no ledger row (a prefilter kill is a standing category rule, not a fresh decision), and NO mention in any Slack alert or digest (Tom, 2026-07-20: "prefiltered folks shouldn't be revealed in alert"). Record the kill in the local audit log only: `[PREFILTERED] {name} — {rule id}: {one clause}`. This is the deliberate exception to "every candidate gets a card" — prefilters exist so category-excluded shapes never reach Tom's queue; the store row + audit line keep it auditable. When a rule ALMOST fires (borderline), don't kill — proceed to scoring and note the proximity in the eval's Gaps **in plain English, never the PF-id** (Tom, 2026-09-21: "spell out the PF. I don't know what PF 19 actually is" — write "career tenure well past the optimal founding window (~15.5y)", never "PF-19 boundary, 15.5y"). PF-codes are a store/audit-log convention only; see the Tom-facing jargon ban below (same rule as W-codes).
...
- **Step 6 (chain to drafting)** — REMOVED in v2. Every headless run ENDS by posting a candidate card to `#neg1-sourcing` for the candidate **regardless of verdict** (Tom, 2026-07-20: every candidate — recommended or not — gets its own individual alert; no consolidated batch notification, ever). Card format is the pipeline-agent Task 6 step 2b anatomy — READ THAT BLOCK and follow it exactly; every 07-27 card drifted from it. The non-negotiables: **the ENTIRE header line is BOLD** (Tom, 2026-08-12: "can you also bold the first line") with the verdict emoji OUTSIDE the bold, everything after it inside — `{🟢|🟡|🔴} **[{COLD|WARM}] {Name} · {current role} @ {current company}{, ex-{spike-era company} if different} · {City}**{ | [Notion]({row URL}) if an Opp/row exists}`. The header emoji carries the verdict as a colored circle (Tom, 2026-07-27): `🟢` = Reach Out, `🟡` = Second Look, `🔴` = Pass — ONLY these three, never ✅/🤔/❌ and never decorative emojis. Concrete: `🟢 **[COLD] Brandon Leahy · Forward Deployed Engineer @ Augment · Toronto**`. **NEVER emit a literal `?` in the role/company slots** (Tom, 2026-09-21, on the Anna Folinsky card that read `? @ ?, ex-ASAPP`): the `?` only appeared because no experience entry was flagged current — the resolver found no ongoing role and left both slots blank. When there is NO current role (the person's most-recent stint has an end date / no `is_current: true` entry), the role/company slot becomes **`ex-{most-recent role} @ {most-recent company}`** built from the last Arc entry — AND the trailing `, ex-{spike-era company}` slot is DROPPED (no double "ex-"). So Anna renders `🔴 **[COLD] Anna Folinsky · ex-MTS Human Data Ops @ Anthropic · Rochester**`, not `? @ ?, ex-ASAPP`. The trailing `, ex-{spike-era company}` slot is used ONLY when a real current role fills the primary slot and the spike company differs from it. Same shape as the `✉️ **Drafted: -1 ({Name})**` close-loop reply, so every Tom-facing header in #neg1-sourcing reads the same way. **This card is an intentional exception to the shared alert convention** (`~/.claude/skills/send-alert/references/alert-convention.md`) — the colored-circle verdict is a deliberate at-a-glance triage signal for the #neg1-sourcing firehose and overrides the "one domain emoji" rule; see that file's Exceptions note.

**NO `Claude Rec.` LINE ON THE SLACK CARD** (Tom, 2026-08-12: *"why do you need claude rec if the green yellow red circles in first line exist"* — restating the 2026-07-20 no-Verdict-bullet rule, which drifted back in). The colored circle in the bolded header IS the verdict; a second line spelling it out is pure redundancy at the top of the card, where Tom's eye lands first. **Where the confusion comes from, so it stops recurring:** the EVAL NOTE template (v3) legitimately opens its callout with underlined-bold `Claude Rec.` + verdict, and this doc describes that callout as "the Slack card 1:1". They are 1:1 in BULLET CONTENT — Presence / Arc / Spike(s) / Gap(s) / Timing — not in the verdict treatment, and the card's leading **Source** bullet (2026-08-29) is CARD-ONLY: the eval-note "no provenance" rule stands, Source never migrates into the Notion callout. The Notion callout needs the words because a Notion page has no colored-circle header; the Slack card has one, so it needs the emoji only. Card goes straight from the bolded header line into the leading **Source** bullet. Header emoji carries the verdict as a colored circle (Tom, 2026-07-27): `🟢` = Reach Out, `🟡` = Second Look, `🔴` = Pass — ONLY these three, never ✅/🤔/❌ and never decorative/thematic emojis (the 2026-07-27 drift: 🧠🧬🎮🤖 headers made verdicts illegible at a glance). There is NO separate Verdict bullet (Tom, 2026-07-20: emoji in the first line is enough). `[COLD]` vs `[WARM]` is decided SOLELY by network-cache membership — trust the store row's `type` (the sweep checks the cache); if unset, check `~/.claude/scripts/network_cache.db` `profiles.linkedin_url` by slug (Tom, 2026-07-27: "warm is when someone exists in the network cache, cold is if not") — never infer it from the relationship read. NO Path bullet, NO Verdict bullet, NO raw signals-line/code block on the card. Bullet order Source → Presence → Arc → Spike(s) → Gap(s) → Timing. **Source is the FIRST bullet, straight after the header, always present** (Tom, 2026-08-29: "in the neg1 slack notification can you include source fields" + "source should be above presence in alert"): `- **Source.** {label}` — the store row's `source` is either an EMAIL (external referrer, human or machine → resolve People DB by Email; display the person's name, or the firm's short common name: "Primary", "Fika") or engine-internal (→ "Claude"). No registry file — the People DB IS the lookup (Tom: "primary and Fika have their own email lookup in people db"). Email NOT resolvable in the People DB → show the raw email + `(not in People DB)` marker — Tom's cue that the referrer is ungated; NEVER auto-create the person. Name or one word otherwise, no commentary. **The LinkedIn URL BELONGS IN THE PRESENCE BULLET, AS THE FIRST ENTRY — never as a trailing bare link at the bottom of the card** (Tom, 2026-08-12, on the Doug Na card: *"btw linkedin should be presence sectino"*). Exact shape: `- **Presence.** [LinkedIn]({LI URL}) · [GitHub (@handle)](url) · [Personal site](url)` — dot-separated, every entry an embedded markdown link, LinkedIn always first even when the rest is thin. Two recurring drifts this kills: (1) writing Presence as GitHub-only prose (`GitHub (@dougkna) — 12 repos incl. …; no public writing found`) and dangling the LinkedIn URL at the end of the card; (2) posting any bare URL, which makes Slack render a link-preview card underneath and buries the eval. If nothing but LinkedIn was found, the bullet is just `- **Presence.** [LinkedIn]({LI URL})` — commentary about what was NOT found belongs in Gap(s), not Presence.

**ROOT CAUSE, name it so it stops recurring:** the skill runs an *Online Presence research* step (Step 4.5) whose job is discovering public ARTIFACTS — writing, GitHub, podcasts — and that research legitimately returns nothing for plenty of people. The card's Presence bullet is a DIFFERENT thing: it is the set of LINKS TO THIS PERSON, and **the LinkedIn URL is always known, because it is the input to the entire enrichment.** Conflating the two is what produced three cards on 2026-08-12 whose Presence read "No public social presence found" / "No public writing, blog, podcast, or GitHub found — four-round search came up empty" **with no LinkedIn link anywhere on the card at all** (Tom: *"and how come there are no LI links for these"*). A null research result must never empty the Presence bullet. Compose it as: LinkedIn link first, always; then any discovered artifacts; then, if the research came up empty, say so in **Gap(s)** — "no public writing or code found" is a real signal about Rigor and belongs there, where it is scored. Numeric scores stay off the bullets ("Reps (9/10)" belongs in eval notes' Signals table only). "Earned Reps" displays as "Reps". Timing in plain good-time-or-not language, never W-codes, never re-check cadence suggestions. Save the posted ts via `set-state --card-ts` for all three verdicts (Tom can still reply `draft` on a card the rubric scored `Pass` — see manual-mode override elsewhere in this doc).
  - **Post ONE message per candidate, in its own Slack message — never batch multiple candidates into a single consolidated post.**
  - **Channel routing (gate in code, NOT the default `send-alert` skill):** post via `send-alert/md_to_blocks.py` in bot-token mode: `SLACK_BOT_TOKEN_FILE=$HOME/.claude/skills/claude-alerts-listener/.bot_token SLACK_CHANNEL=$(cat ~/.claude/skills/neg1-sourcing/.sourcing_channel_id) BODY_FILE=<tmpfile> python3 ~/.claude/skills/send-alert/md_to_blocks.py` (prints the message `ts` — no webhook needed). Do **not** fall back to the generic `send-alert` skill for this card — that skill's webhook is fixed to `#claude-alerts` and will misroute the candidate card there (this was the 2026-07-20 bug Tom flagged: per-candidate cards were landing in `#claude-alerts` instead of `#neg1-sourcing`). If `.sourcing_channel_id` is missing, skip posting and note the candidate in the run summary instead.

**Job mode (per-candidate, event-driven — the PRIMARY scheduled path):** invoked via claude-job-queue with args `{mode: "headless", li_url, source, name?}` (enqueued by `~/.claude/scripts/enqueue-neg1-enrich.sh` the moment a generator surfaces a name — there is NO batching delay). Run the full headless flow above for the single candidate, card it regardless of verdict, exit. Do NOT post a separate "run complete" / job-summary message anywhere — the candidate card (or the Step-0 existing-Opp notice) IS the completion signal; job telemetry stays in the queue + audit logs (Tom, 2026-07-27). If the job fails before carding, post a failure note to `#neg1-sourcing` (bot-token mode), not `#claude-alerts`. **EXACT shape — the STANDARD alert grammar (`send-alert/references/alert-convention.md`), NOT the candidate-card exception** (that exception is scoped to cards carrying a verdict; a failure note has none):
  ```
  🧍 <u>**Enrichment Failed: {Name}**</u>
  ✗ {plain-English reason, one clause}
  **Source:** {resolved label}
  [LinkedIn]({LI URL})
  ```
  Rules (locked 2026-09-18 after a drifted alert shipped a ⚠️ header, a broken nested-parens link, a raw source email, and a job_id): (1) domain emoji is `🧍` (person lane, from the convention's closed table) — never `⚠️`/`❌` emoji forms; failure state is the plain `✗` glyph leading the outcome line; (2) **Source** resolves exactly like the card's Source bullet — People DB by email → short firm name ("Primary", "Fika"), engine-internal → "Claude", unresolvable → raw email + `(not in People DB)`; (3) LinkedIn is a markdown `[LinkedIn](url)` footer link below the rows — never a bare URL (spawns a Slack preview card), never nested inside other parens; (4) NO job ids, queue ids, or telemetry of any kind — the queue + audit logs own that (same no-audit-jargon rule as every operator-facing alert). Idempotency: if the store row for `li_url` is already `state=surfaced` with a `card_ts` (or any later state), exit silently — the job is a retry.
...
- **Apply the When gate (RUBRIC.md §8.9) AFTER the verdict.** From `experience[]` career-state evidence, check:
  ...
  If W1 or W2 fires AND Claude Rec is `Reach Out ✅`: keep the pre-gate rec in the Signals `rec:` element, downgrade `Claude Rec` to `Second Look 🤔`, and prepend one PLAIN-LANGUAGE timing line to Eval Summary — `Timing: not the moment – <one-line evidence in plain words>.` If only W3 matches: keep the rec unchanged and append `Timing note: <plain-language evidence>.` — NO re-check/re-look cadence suggestion (Tom, 2026-07-20: "leave it to me to figure out when I'm going to reach out again"; he replies `track <dur>` when he wants a re-surface). **Never write W1/W2/W3 codes, "When gate" phrasing, OR PF-ids (PF-19 etc.) into Eval Summary, the card Gaps/Timing line, or any other Tom-facing text** (Tom, 2026-07-20: "I have no idea what it actually means; just tell me if it's a good time to reach out or not and why"; reaffirmed 2026-09-21 on a PF-19 leak: "spell out the PF. I don't know what PF 19 actually is") — the W-codes live ONLY in the store's `when_gate` field and the PF-ids ONLY in the store/audit log, both for ledger back-testing. Spell the reason out in plain English instead ("career tenure well past the optimal founding window"). The gate never auto-passes, never changes signal scores, and never fires on someone already in a founder-shaped seat (founding now, running their own firm, or visibly between stints exploring). The `rec:` element of the Signals line is the PRE-gate auto-rec — that is what lets the ledger back-test the gate itself.

...
- **Body = the "Tier A" mini-memo** (Tom-approved 2026-07-16; prototyped on Stephanie Wan). Notion-native only — no rendered images/PDFs. Blocks in this exact order:
  1. **Verdict callout** — `<callout icon="🧠" color="{verdict color}">` — icon is ALWAYS the 🧠 brain emoji (Tom's pick, 2026-07-17), background color-coded by verdict: Reach Out→`green_bg`, Second Look→`yellow_bg`, Pass→`red_bg`. Inside, three lines: `**Claude Rec: {verdict}**` (verdict as plain text, NO emoji after it and no parenthetical), then a 2–3 sentence summary + re-surface/next-check line, then an italic provenance line `*Enriched from {sources}. {YYYY-MM-DD}.*` as the callout's last line (Tom moved it here from a footer quote, 2026-07-17 — there is NO standalone footer block). The callout IS the summary — no separate `**Summary.**` paragraph.
  2. **`#### Profile`** — three bullets (Tom's format, 2026-07-17): `**Current Seat:** {role}, {company} ({city})`; `**Career Arc:** Company (Role, years) → Company (Role, years) → …` (work stops only, each with role + years in parens); `**Education:** School (years) → School (years)`. No Contact/Home/Source here — those live on the Opp properties.
  3. **`#### Signals` table** (header-row) — columns `Signal | Score | In one line`. Score = plain `{n}/10` (NO dot scales — Tom found them weird). Peak signal bolded with a trailing `(Spike)` label (not a ★). Signal names spelled out in full (never NL/Ant/Rng) — EXCEPT "Earned Reps", which is always displayed as just "Reps" on every Tom-facing surface (Tom, 2026-07-20). Numeric scores live ONLY in this table — the prose below must NOT repeat `({n}/10)` score-led paragraphs.
  4. **`#### Founder Eval`** — six per-signal BULLETS (`- **{Signal}.** prose` — bulleted list, not bare paragraphs; Tom, 2026-07-17), each led by the bolded signal name (`- **Intellectual Rigor (Spike).**`, `- **Intentionality.**`, …), ordered by score descending so the spike leads. NO What/How/Why lens rollup headings (Tom tried it and rejected it, 2026-07-16) and NO `({n}/10)` in the bold leads — scores stay in the table. Each paragraph: evidence + why-this-level in prose; the spike gets the most ink.
  5. **`#### Other Qualities`** — two bullets: `- **Standout Detail.** {one memorable concrete fact — the line Tom would repeat to someone else}` and `- **Closest Founder Archetype.** {one line mapping to the §6 four-archetype taxonomy, or "too early to assign"}`.
  6. **`#### Potential Gaps`** — 2–4 falsifiable bullets: evidence that would move the scores (a 0→1 seat, an opportunity cost walked past, fuller enrichment). This is evidence-framing, NOT a reach-out recommendation.
  All section headings are heading-4 (`####`) — never `##`/`###` (Tom, 2026-07-17). NO standalone footer block after Potential Gaps — provenance lives as the callout's italic last line (see item 1).
...
- **Step 6 (chain to drafting)** — REMOVED in v2. Every headless run ENDS by posting a candidate card to `#neg1-sourcing` for the candidate **regardless of verdict** (Tom, 2026-07-20: every candidate — recommended or not — gets its own individual alert; no consolidated batch notification, ever). Card format is the pipeline-agent Task 6 step 2b anatomy (read that block for the exact card anatomy — incl. the 2026-07-20 rulings: header = `{emoji} [COLD|WARM] Name · role @ company · City` (dot-separated, no parens) with no Path or Verdict bullets; bullet order Presence → Arc → Spike(s) → Gap(s) → Timing; "Earned Reps" displays as "Reps"; Timing in plain good-time-or-not language, never W-codes, never re-check cadence suggestions). Save the posted ts via `set-state --card-ts` for all three verdicts (Tom can still reply `draft` on a card the rubric scored `Pass` — see manual-mode override elsewhere in this doc).
...
**Job mode (per-candidate, event-driven — the PRIMARY scheduled path):** invoked via claude-job-queue with args `{mode: "headless", li_url, source, name?}` (enqueued by `~/.claude/scripts/enqueue-neg1-enrich.sh` the moment a generator surfaces a name — there is NO batching delay). Run the full headless flow above for the single candidate, card it regardless of verdict, exit. Idempotency: if the store row for `li_url` is already `state=surfaced` with a `card_ts` (or any later state), exit silently — the job is a retry.

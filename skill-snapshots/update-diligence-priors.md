...
- **Diligence Materials** — check for new Google Drive files, PDFs, or attachments not referenced
  ...
  **HARD RULE — every company-provided material is in scope, regardless of filename labels.**
  Do NOT skip files because the filename or title contains `[DEPRECATED]`, `(deprecated)`,
  `OLD`, `[OLD]`, `superseded`, `v0`, `archived`, `graveyard`, or any other founder-applied
  "this is no longer current" tag. The only valid filters when building the new-materials set
  are (a) is this already in the prior-processed set, and (b) is this the diligence output
  this skill itself wrote (e.g., the prior `_v[N].pdf` snapshot). Nothing else. Founder-marked
  deprecation is itself diligence signal — include the file with a `(deprecated)` qualifier
  inline, and use the contrast between deprecated and current as material for the Prior
  Assessment (what changed, what was walked back, what the rename or pivot signals about
  founder thinking). Memory: `feedback_diligence_materials_deprecated_not_skip`. Caught after
  AgentBay 2026-06-25 first update-priors run silently dropped the merchant pitch deck
  because Nipun had labeled it `[DEPRECATED]` in the file title — the deck contained the
  prior "Shopper Intent Score (SIS™)" branding (since renamed PINT/ALE) and the original
  90-day merchant design-partner timeline, both substantive evidence of how the company
  thinking evolved between first-pass and update.

  **Classification rule.** All company-provided artifacts (decks, financial models, docs,
  slides, transcripts, attachments, screenshots founders sent) classify under **Materials**
  in the New Information Processed list — never under "Customer Feedback" or other
  reader-feedback categories. "Customer Feedback" is reserved for third-party NPS / churn /
  customer-interview notes generated by parties other than the company. Tom-authored
  framework docs (Diligence Q&A, question lists, analytical scratchpads, pre-mortems) go
  under **Research & Analysis**, not Materials. If the file's Drive owner is a company-domain
  email (e.g., `@founder-gmail.com`, `@company.com`), classify as Materials; if owner is
  `tom@invertedcap.com`, classify as Research & Analysis.

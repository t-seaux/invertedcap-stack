...
2. Scan iMessages via `get_unread_imessages`. May fail (Full Disk Access) — skip gracefully if so.
   **CONFIRM-GATE GUARD (hard block — added 2026-09-21 after the MPOWR incident):** Only messages written by a *human* count as signal. NEVER treat a message authored by our own alert bot (KSeo Bot / Sendblue, e.g. "🆕 Opportunity: <Name> … 👍 to Add to CRM") as a deal signal — that bubble is a proposal *awaiting Tom's approval*, and consuming it as inbound creates the Opp without his 👍. Concretely, before creating anything: (a) skip any iMessage whose author is the bot / whose text contains "👍 to Add to CRM"; (b) list `~/.claude/skills/deal-text-scanner/staged/*.json` and skip any candidate whose `company` / `opp_title` / `contact` matches a staged payload — those are confirm-gated by design and only add-to-crm may create them on Tom's 👍. Note each as "skipped — pending Tom's 👍 (staged): [name]". A deal found ONLY via the bot's proposal and not in a primary source you read yourself is never created here.
...
2b. **Surface EVERY candidate to `#neg1-sourcing`** (Tom, 2026-07-20 — every candidate gets its own individual card regardless of verdict; no consolidated batch notification): for each row enriched in THIS run, post ONE candidate card, in its own separate Slack message, to the `#neg1-sourcing` Slack channel — post via `send-alert/md_to_blocks.py` in bot-token mode: `SLACK_BOT_TOKEN_FILE=$HOME/.claude/skills/claude-alerts-listener/.bot_token SLACK_CHANNEL=$(cat ~/.claude/skills/neg1-sourcing/.sourcing_channel_id) BODY_FILE=<tmpfile> python3 ~/.claude/skills/send-alert/md_to_blocks.py` (prints the message `ts` — no webhook needed). Do **not** use the generic `send-alert` skill for this card — its webhook is fixed to `#claude-alerts` and will misroute it (the 2026-07-20 bug: cards were landing in `#claude-alerts` one-per-candidate instead of `#neg1-sourcing`). Card format (Tom's mock, locked 2026-07-16; verdict line added 2026-07-20 now that Second Look/Pass cards also post — labeled bullets):
   ...
   - **Source.** {email `source` → the referrer's NAME from their People-DB row (People DB by Email is the lookup; Primary/Fika resolve this way too — display the short common name, "Primary", "Fika"); engine-internal → Claude. Email NOT resolvable in the People DB → show the raw email + marker: `jane@acme.com (not in People DB)` — that marker doubles as Tom's cue that the referrer is ungated; never auto-create the person (Tom, 2026-08-29). Name or one word otherwise, no commentary — provenance, not evaluation. (Tom, 2026-08-29; moved ABOVE Presence same day: "source should be above presence in alert")}
   - **Presence.** [LinkedIn]({LI URL}) · {[Platform (@handle)](url) · … additional artifacts, dot-separated}
     LinkedIn is ALWAYS the first entry and always lives HERE — never as a trailing bare link at the
     bottom of the card (Tom, 2026-08-12: "btw linkedin should be presence sectino", on a card whose
     Presence bullet was GitHub-only prose with the LI URL dangling at the end). Every entry is an
     embedded markdown link; a bare URL makes Slack render a preview card that buries the eval. When
     nothing else was found the bullet is just `[LinkedIn](url)` — notes about what was NOT found
     (no writing, thin footprint) belong in Gap(s), not Presence.
   ...
   ```
   Bullet order is FIXED: Source → Presence → Arc → Spike(s) → Gap(s) → Timing (Arc before Spike, Timing LAST — Tom, 2026-07-20; Source bullet added 2026-08-29, FIRST above Presence per Tom same day).
   Header anatomy (Tom's format, 2026-07-20 final: `[COLD] Varun Joshi · Head of Engineering @ Patronus AI · San Francisco`): **The ENTIRE header line is BOLD** (Tom, 2026-08-12: "can you also bold the first line") — verdict emoji OUTSIDE the bold, everything after it inside: `🟢 **[COLD] Brandon Leahy · Forward Deployed Engineer @ Augment · Toronto**`. Same shape as the `✉️ **Drafted: -1 ({Name})**` close-loop reply, so every Tom-facing header in #neg1-sourcing reads the same way.

**NO `Claude Rec.` LINE ON THE SLACK CARD** (Tom, 2026-08-12: *"why do you need claude rec if the green yellow red circles in first line exist"* — restating the 2026-07-20 no-Verdict-bullet rule, which drifted back in). The colored circle in the bolded header IS the verdict; a second line spelling it out is pure redundancy at the top of the card, where Tom's eye lands first. **Where the confusion comes from, so it stops recurring:** the EVAL NOTE template (v3) legitimately opens its callout with underlined-bold `Claude Rec.` + verdict, and this doc describes that callout as "the Slack card 1:1". They are 1:1 in BULLET CONTENT — Presence / Arc / Spike(s) / Gap(s) / Timing — not in the verdict treatment, and the card's leading **Source** bullet (added 2026-08-29) is CARD-ONLY: the eval-note template's "no provenance" rule stands, so Source never migrates into the Notion callout. The Notion callout needs the words because a Notion page has no colored-circle header; the Slack card has one, so it needs the emoji only. Card goes straight from the bolded header line into `**Presence.**`. verdict emoji first, then a `[COLD]`/`[WARM]` tag, then name / role @ company / city as ` · `-separated segments — NO parens around the role (Tom, 2026-07-20: "instead of parens for the role separate with dots"). The tag replaces the Path bullet — there is NO Path bullet anymore; for warm candidates with a known route, fold it into the tag (`[WARM – via {name}]`, ≤4 words). Keep `, ex-{spike-era company}` after the company only when the spike company differs from the current one (2026-07-16 ruling: a spike-era-only header read as stale data — Tom: "you didn't get her latest info"). **When there is NO current role** — the person's most-recent stint has an end date, or no experience entry is flagged `is_current: true` — do NOT emit a literal `?` in the role/company slots (Tom, 2026-09-21, on the Anna Folinsky card that read `? @ ?, ex-ASAPP`): fill the primary slot from the LAST Arc entry as `ex-{most-recent role} @ {most-recent company}` and DROP the trailing `, ex-{spike-era company}` slot entirely (no double "ex-"). Anna renders `🔴 **[COLD] Anna Folinsky · ex-MTS Human Data Ops @ Anthropic · Rochester**`. The trailing `, ex-{spike-era company}` slot is used ONLY when a real current role fills the primary slot and the spike company differs. Header emoji carries the verdict as a colored circle (Tom, 2026-07-27): `🟢` = Reach Out (recommend), `🟡` = Second Look (want a second opinion), `🔴` = Pass — ONLY these three, never ✅/🤔/❌ and never decorative/thematic emojis (the 2026-07-27 drift: 🧠🧬🎮🤖 headers made verdicts illegible at a glance). There is NO separate Verdict bullet (Tom, 2026-07-20: emoji in the first line is enough). The `[COLD]`/`[WARM]` tag is DETERMINED BY NETWORK-CACHE MEMBERSHIP, nothing else (Tom, 2026-07-27: "warm is when someone exists in the network cache, cold is if not") — check `~/.claude/scripts/network_cache.db` (`profiles.linkedin_url`, slug match) or trust the store row's `type` which the sweep set the same way; NEVER re-derive warm/cold from vibes, path prose, or relationship guesses (the 2026-07-27 bug: two network-cache reconnects were carded `[COLD]`). Presence is the SECOND line (first bullet), always present, leading with the LinkedIn link, entries dot-separated (Tom, 2026-07-20). Labels are bold — `- **Spike(s).**` — plain bullets, NO brackets, NO `--` dividers, no blank lines anywhere (blank lines emit `\n\n` spacers). NO verb-legend footer — the reply grammar lives in the pinned channel legend and the listener parses free text anyway; the `[neg1:<short-id>]` fingerprint is the only footer line (the listener needs it to resolve the row). Short signal names on cards: Reps / Non-Linearity / Rigor / Anticipation / Intentionality / Range — "Earned Reps" is ALWAYS displayed as just "Reps" on any Tom-facing surface (Tom, 2026-07-20); the full name survives only in RUBRIC.md doctrine and internal store fields.
...
**v2 (CURRENT — candidate store):** candidates to check are store rows with `state=drafted` (`candidates.py list --state drafted`), matched against Gmail sent mail by recipient email / name. On a detected send: (1) `set-state --state reached-out`; (2) **flip the EXISTING Opp's Status `Qualified` → `Outreach`** — the Opp was created at draft time with Status Qualified (Tom, 2026-07-27: "only if i actually reach out to her should you add to outreach"; the send IS the reach-out moment). NO add-to-crm bridge — the Opp already exists (`notion_opp_url` on the store row); creating another is the duplicate bug class. (3) Ledger: append `--decision reached-out --source "-1 scanner"` per the reach-out-at-send rule. Everything below is the LEGACY Scanner-era flow (Draft Ready rows + add-to-crm bridge at send) — still valid only for pre-2026-07-16 scanner rows in flight.
...
3. **For each detected send**:
   ...
   **b) Invoke `add-to-crm` to create a pre-company Opportunity** with these exact pre-populated fields:
   ...
   | **Stage** | `Pre-Seed 💡` |
   | **Source(s)** | **Provenance-aware — check the store row's `source` field first** (Tom, 2026-08-29, Charles Wong). TWO shapes, no registry: `source` is an **email address** → resolve the **People DB by Email** (Primary = `deal-agent@primary-os.com`, Fika = `investments@fika.vc`; same lookup as any human referrer) → that row; email not found → Claude + flag the gap, never auto-create. Anything else = engine-internal → `["https://www.notion.so/07500beff4aa8213a8f801cfa3cb9a12"]` — the **Claude** source page (Tom, 2026-07-27: every candidate sourced through the engine gets Source = Claude). ⚠️ NOT `0fb9a640…` — that is **Direct**'s page, the exact 2026-08-03 copy-paste bug documented in neg1-sourcing-listener/SKILL.md; it survived here until the 2026-08-11 review |

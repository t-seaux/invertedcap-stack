...
**Referrer's own framing IS a stage signal — read the whole thread, not just the pitch line.** When the source is a text/DM thread, the referrer's setup message often states the stage before the company is even named (e.g. Matt Wong: "are you still making pre-seed investments out of Inverted? I met someone today I wanted to introduce you to" — the WealthStone Opp this precedes IS pre-seed, stated, not defaulted). Read every message in the thread, not just the one with the company name/link, and treat a referrer's own stage-framing as a confirmed Stage rather than falling back to the `Pre-Seed 💡` default and hoping it lines up (Tom, 2026-09-24 — "you should've known wealthstone was pre seed based on my text thread with Matt").
...
## Step 4T: Text-Card Gate (webhook mode, new deal only)
...
- Prior pass / terminal status — no Slack alert here; the revive gate's 🔁 text card covers it (`shared-references/revive-gate.md`).

...
- **Any candidate has Status ∈ {Pass (Met), Pass (DNM), Lost, NR / Missed}** — do NOT create a duplicate, and do NOT silently reactivate or edit the row. Run the **Revive Gate** (`~/.claude/skills/shared-references/revive-gate.md`): write NOTHING to the existing row now — STAGE this inbound's new info (fresher `Description`, dated body note, any new materials) AND the proposed reactivation, then **propose it 👍-gated via a 🔁 revive TEXT card** to Tom (`+12012567714`) + a staged `action:"revive"` payload. `target_status` = where this inbound would land per Step 5 inference: a founder re-pitching directly → `Connected`; a **referrer offering to (re)introduce** Tom → `Outreach` (the intro hasn't happened — do not skip to Connected). The revive proposal is TEXT even for this email channel — that is where Tom's 👍 applies the info + flips the status (sms-listener §4); nothing touches the passed row before the 👍. This **supersedes the old `⛔`/`prior-pass-skip` Slack path**: the caller (`inbound-deal-detect`) does NOT post a ⛔ Slack alert; the text revive card is the notification. Log `prior-pass-revive-proposed` with the existing page ID and exit 0.

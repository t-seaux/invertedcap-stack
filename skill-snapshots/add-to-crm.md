...
- **Any candidate has Status ∈ {Pass (Met), Pass (DNM), Lost, NR / Missed}** — do NOT create a duplicate, and do NOT silently reactivate. Run the **Revive Gate v2** (`~/.claude/skills/shared-references/revive-gate.md`, Tom 2026-09-22): **ENRICH the existing row NOW** — run Step 6 (`materials-handler`) against the existing page for any attachment / `materialUrls` so the deck lands in `Diligence/<Company>/` and on `Diligence Materials`; append a `## Update (YYYY-MM-DD)` section to the body (one-line signal + source context + Gmail thread link); READ the deck (Step 1B) and refresh `Stage` + `Round Details` from it; fill `Description` only if blank/`TBD`. Do NOT touch `Status`. Then STAGE the reactivation (`action:"revive"` payload with the `enriched` block) and **text Tom the 🔁 revive card** (`+12012567714`) — the card says what was enriched and asks only for the revive 👍. `target_status` = where this inbound would land per Step 5 inference: a founder re-pitching directly → `Connected`; a **referrer offering to (re)introduce** Tom → `Outreach` (the intro hasn't happened — do not skip to Connected). The card is TEXT even for this email channel — that is where Tom's 👍 flips the status (sms-listener §4). No ⛔ Slack post (the old `prior-pass-skip` path is retired); the materials chip fires its own Slack alert and the text card is the notification. Log `prior-pass-enriched-revive-proposed` with the existing page ID and exit 0. Webhook runs arrive with `knownTerminalOpp` already resolved by `deal-scanner.js` — use it as the primary dedup candidate, but still run the queries above.
- **Any candidate is in an in-progress status (Qualified / Outreach / Connected / Scheduled / Active / Track / Exploration / Assigned / Pass Note Pending)** — this is a re-surface of an existing live deal. Do NOT create a duplicate. In manual mode, surface the existing page and ask whether to update it in place. In unattended mode, **first run the completeness check below**; only if it passes, log `duplicate-in-pipeline-skip` with the existing page ID and exit 0.
  ...
  >
  > This matters because the skip is otherwise *silent*: the 🔁 duplicate alert is suppressed for exactly these in-progress statuses, so a killed run leaves an Opportunity with no deck, no verification, and **no Slack notice to Tom**. That failure is worse than a visible duplicate — a duplicate gets noticed and merged, whereas a half-built row looks finished. Same principle `add-to-companies` already gets right by keeping row-exists and `Last Enriched` as separate signals, so a partial run is backfilled on re-entry instead of skipped.
...
- `Status`: Infer from the full thread state. **If the caller (e.g. `inbound-deal-detect`) passed an explicit `status` directive, honor it verbatim — do not re-infer.** Otherwise apply the defaults below:
  ...
  - **`Connected` — cold inbound founder email** where the founder is the actual sender (deal-scanner webhook path, or `forwardedFromTom: true` AND the inner sender's email domain matches the pitched company). The founder pinged Tom directly and the email landed in his inbox; Tom is in a live thread with the founder from message #1, even before he replies. Do NOT use Qualified for these. **Tom's own addresses** are `tom@invertedcap.com`, `tom@dashfund.co`, `thomas.seo@outlook.com`, and `tseo@primary.vc` (his Primary Venture Partners address) — an inner `To:` at any of these means the founder wrote directly to Tom, so this is a `Connected` cold inbound, not a referrer forward. **Source = Direct in this case** — the company reached out to Tom directly, so there is no referrer even though the email arrived as a `Fwd:` (Tom self-forwarding his own inbound). Never set the forwarding path or Tom's own alias as the Source; use the canonical "Direct" People page.
...
- `Shared` (renamed from `Support`, 2026-08-21 — tracks who a deal was SHARED out to): set to the "N/A" entry at creation: `https://www.notion.so/18200beff4aa80bc8344fc48c7b0fdb1`
...
- **Source(s)**: Identify who referred/sourced the deal from context (email sender, person who texted, person who made the intro, etc.). Search the **People DB only** using `notion-search` with `data_source_url` set to `collection://1715ce8f-7e54-43e2-bbcd-17a5e50cb8c9`. If a matching entry is found, include their page URL in the `Source(s)` property at creation time. If not found, leave blank. **If no source/referrer is identified (e.g. user found the profile directly), default to "Direct": `https://www.notion.so/0fb9a64034fd46f9934768d590e69dc9`.** **Direct-inbound rule: when the company/founder reached out to Tom directly — the inner sender's email domain matches the pitched company (a cold inbound founder email, even if Tom self-forwarded it) — Source is always "Direct". Do NOT treat Tom's own forwarding alias (incl. `tseo@primary.vc`) as a referrer; a self-forward is not a referral.** **Engine-referral aliasing rule (Tom, 2026-08-20): a referral from JORDAN FOX SPECIFICALLY (or from `deal-agent@primary-os.com` itself) is a PRIMARY referral, not a Jordan referral — Jordan operates Primary's AI deal-sharing engine, so those deals come through the engine, not his personal network. Set Source(s) to the "Primary" People page: `https://www.notion.so/3c200beff4aa80079fb5d6680db3702c` — NOT Jordan Fox's person page — even though the email arrives from Jordan's own address. This is a NAMED-PERSON rule, not a domain rule (Tom, same day): other Primary people sourcing personally — Emily Man, Ben Sun, anyone else `@primary.vc` — are normal individual referrals, sourced to their own person page. Same pattern for any future firm engine: alias only the named engine operator(s), never the firm's whole domain. Counter-example for contrast (Tom, same day): Fika has NO sourcing engine — TX Zhuo, Gabriela, or anyone else at Fika referring a deal always sources to their own person page; the "Fika" People pseudo-page (`3c200bef-f4aa-8134-8b3b-d5ca8e46bc0c`) is a deal-share Bcc contact and exclusion anchor ONLY, never a Source, unless Tom later says Fika stood up an engine.**
...
- **Shared**: Always set to `https://www.notion.so/18200beff4aa80bc8344fc48c7b0fdb1` (the "N/A" entry) at creation. Renamed from `Support` 2026-08-21 — it now tracks deal-share recipients: the gmail-webhook `deal-share-sent` handler appends member pages on send, and Tom sets it by hand when he shares manually. Never write anything but N/A here at creation time.
...
6. **Deterministic pre-exit gate (run before the reply summary / webhook exit-0): if HQ is `??? 🌀`, you MUST have already run the full HQ cascade including a `WebSearch` this turn.** If it's `??? 🌀` and the cascade wasn't run, run it now and update the row before finishing — do not exit with an un-investigated HQ. This is a hard, deterministic requirement, not a judgment call, and it is NEVER satisfied by asking Tom whether to look it up — resolve it yourself.

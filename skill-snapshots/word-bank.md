...
name: word-bank
description: |-
  Maintain Tom's personal word bank — words he wants to eventually use in his writing. (C) Manual add — "add to word bank [word]", "add [word] to word bank", "word bank: [word]", "wordbank [word]", or words handed over with save intent → concise definition + natural example appended to word_bank.md. (A) Weekly refresher — "word bank refresher", "refresh my word bank", "what words did I add", "word bank recap", or the scheduled weekly run (Sunday 9 PM ET, texted to Tom's 1:1) → that week's words, each with a grounded retrofit: a real sentence from Tom's investment memos + LP letters rewritten to use the word (corpus sync mechanics in the skill body; zero-new-words cost gate). Single local Markdown file — no Notion/DB. Always trigger inline — no confirmation needed to add.
